...
async function loadDashboard() {
    ...
    if (ctxPlanes) {
        ...
        try {
            const planes = await fetchAPI(planesUrl);
            if (planes && planes.length > 0) {
                ...
                const abiertos = planes.filter(p => {
                    ...
                    return estado !== 'cerrado' && p.fecha_cumplimiento && p.fecha_cumplimiento.trim() !== '';
                });
...
async function verDetalleAuditoria(id, estadoArg) {
    ...
    if (estadoArg === 'Finalizada' || data.estado === 'Finalizada' || data.estado_cierre === 'Cerrado') {
...
function renderGanttPlanes(planes) {
    ...
    const conFecha = planes.filter(p => p.fecha_cumplimiento && p.fecha_cumplimiento.trim() !== '');
...
async function confirmarAprobacionPlanes() {
    ...
    // Generar PDF primero y luego mandarlo al backend
    const element = document.createElement('div');
    element.style.padding = "20px";
    element.innerHTML = `
        <h2>Reporte Final de Planes de Accion</h2>
        <p><strong>Auditoria ID:</strong> ${aud_id}</p>
        <hr>
        <table border="1" cellpadding="5" cellspacing="0" style="width:100%; border-collapse: collapse;">
            ${document.getElementById('tabla-planes-accion') ? document.getElementById('tabla-planes-accion').innerHTML.replace(/<button[^>]*>.*?<\/button>/gi, '').replace(/<input[^>]*>/gi, '').replace(/<textarea[^>]*>.*?<\/textarea>/gi, '(Ver detalle original)') : 'Plan de Accion Aprobado'}
        </table>
        <p style="margin-top: 20px;"><em>Documento validado con firma dual (Prevencionista y Administrador de Obra)</em></p>
    `;
    
    html2pdf().set({
        margin: 0.5,
        filename: `Planes_Auditoria_${aud_id}.pdf`,
        jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' }
    }).from(element).outputPdf('blob').then(async function(pdfBlob) {
...
function exportarResumenAnualExcel() {
    ...
    csvContent += "Año,Mes-Año,Trabajadores,Días Perdidos\n";
    ...
    extractTable('tabla-resumen-anio1', 'Año 1 (Jul 2024 - Jun 2025)');
    extractTable('tabla-resumen-anio2', 'Año 2 (Jul 2025 - Jun 2026)');
    extractTable('tabla-resumen-anio3', 'Año 3 (Jul 2026 - Jun 2027)');
